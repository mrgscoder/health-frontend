...
const BloodPressureTracker = () => {
  ...
  const categorizeBP = (sys: number, dia: number) => {
    if (sys >= 180 || dia >= 120) {
      return { category: 'Hypertensive Crisis', color: 'bg-red-600', textColor: 'text-red-600', urgency: 'URGENT' };
    } else if (sys >= 140 || dia >= 90) {
      return { category: 'High BP Stage 2', color: 'bg-red-500', textColor: 'text-red-500', urgency: 'HIGH' };
    } else if ((sys >= 130 && sys <= 139) || (dia >= 80 && dia <= 89)) {
      return { category: 'High BP Stage 1', color: 'bg-orange-500', textColor: 'text-orange-500', urgency: 'MODERATE' };
    } else if (sys >= 120 && sys <= 129 && dia < 80) {
      return { category: 'Elevated', color: 'bg-yellow-500', textColor: 'text-yellow-600', urgency: 'WATCH' };
    } else if (sys < 120 && dia < 80) {
      return { category: 'Normal', color: 'bg-green-500', textColor: 'text-green-600', urgency: 'GOOD' };
    } else {
      return { category: 'Check Values', color: 'bg-gray-500', textColor: 'text-gray-600', urgency: 'REVIEW' };
    }
  };
